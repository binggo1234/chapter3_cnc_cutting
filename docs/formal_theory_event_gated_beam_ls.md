...
`min_A F(A), subject to S subseteq Coverage(A) and H(A) = 0`
...
实际实现中，覆盖选择器优先选择高价值复合单元，并跳过已完全被覆盖的冗余单元；少数复合单元可能与已覆盖边段发生部分重叠。动态加工状态 `P_k` 记录已加工边段，用于释放、稳定性和入刀状态更新，避免把这类重叠解释为漏切。因而本文主张的是原始轮廓完备覆盖和工艺状态一致性，而不是严格的集合精确覆盖。

这一定义体现本文第一个贡献：算法不是在原始边段上排序，而是先把可合并的边界关系转化为异构候选切割单元。共边、近共边和同线链单元降低的是真实加工长度和重复加工动作，而不只是几何空移距离。
...
- `N_rep(A)` 是已加工原始边段被再次切割的次数；
- `L_rep(A)` 是重复切割对应的原始边段长度；
- `N_redun(A)` 是完全不引入新覆盖边段的冗余切割动作数；
...
`J(A) = M_1 H(A) + M_2 Phi_stab(A) + M_3 N_rep(A) + M_4 L_rep(A) + M_5 N_redun(A) + M_6 C_mach(A) + M_7 N_tool(A) + C_travel(A) + w_a D_air(A) + w_theta Theta(A) - w_r R_cont(A)`
...
代码实现保留 `repeat_cut_policy` 消融开关：`hard` 将 `N_rep`、`L_rep` 和 `N_redun` 置于 `C_mach` 之前，优先消除重复加工；`soft` 将 `L_rep` 作为加工代价的附加项，并用重复次数作次级判别；`off` 忽略重复加工专项惩罚，用于复现实验中的非重复约束基线。
...
若 `U` 包含所有原始边段对应的 `single_edge` 单元，且最终选择满足覆盖约束 `sum_{u: s in C(u)} x_u >= 1`，则输出路线完整覆盖所有原始轮廓边段，不会产生漏切。共边、近共边和同线链单元作为覆盖增强项进入选择，不破坏原始轮廓覆盖完备性；可能存在的部分重叠由动态已加工边段集合 `P_k` 记录和处理。

证明思路：对任意 `s in S`，`single_edge(s)` 保证候选存在性；覆盖不等式保证每条原始边段至少被一个候选单元覆盖；方向变量只改变执行方向，不改变覆盖集合 `C(u)`；状态变量 `P_k` 在执行过程中记录已加工边段，使释放与稳定性判断基于实际加工进度更新。

### Proposition 2: Monotonicity of process-aware local refinement

工艺局部搜索以 `F_proc` 为接受准则，只有当邻域解 `A'` 满足 `F_proc(A') <_lex F_proc(A)` 时才替换当前解。因此，搜索过程中的目标向量严格单调下降，并在有限邻域集合上终止于相对于给定邻域的局部词典序最优解。
